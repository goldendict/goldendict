...
public:
    ...
    string process() {
        ...
        QString ss;
        ss.reserve(_data.size() + 64);
        ss = "<div class=\"sdct_k\">";
        if (!doc.load_string(_data.toUtf8().data())) {
            ss += _data ;
        } else {
            QStringList sl;
            walkNode(doc.first_child(), sl);
...
private:
#ifdef GD_PUGIXML_XSERIAL
    void walkNode(const pugi::xml_node& e, QStringList& sl) {
        if (e.empty()) {
            return;
        }
        if (e.type() == pugi::node_pcdata) {
            sl.append(e.value());
        } else {
            pugi::xml_node n = e.first_child();
...
void StardictDictionary::makeFTSIndex( AtomicInt32 & isCancelled, bool firstIteration )
{
