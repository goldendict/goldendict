...
void MediaWikiWordSearchRequest::downloadFinished()
{
    ...
    if ( netReply->error() == QNetworkReply::NoError )
    {
        ...
        if ( !dd.load_buffer( bac.data(), bac.size() ) )
        {
        ...
        {
            pugi::xml_node pages = dd.child( "api" ).child( "query" ).child( "allpages" );

            if ( !pages.empty() )
            {
                pugi::xpath_node_set nl = pages.select_nodes( "p" );

                Mutex::Lock _( dataMutex );

                for( Qt4x5::Dom::size_type x = 0; x < nl.size(); ++x )
                    matches.push_back( gd::toWString( nl[ x ].node().attribute( "title" ).value() ) );
            }
        }
#else
        QDomDocument dd;
...
void MediaWikiArticleRequest::requestFinished( QNetworkReply * r )
{
    ...
    for( ; netReplies.size() && netReplies.front().second; netReplies.pop_front() )
    {
        ...
        if ( netReply->error() == QNetworkReply::NoError )
        {
            ...
            {
                pugi::xml_node parseNode = dd.child( "api" ).child( "parse" );

                if ( !parseNode.empty() && parseNode.attribute( "revid" ).as_int() != 0 )
                {
                    pugi::xml_node textNode = parseNode.child( "text" );

                    if ( !textNode.empty() )
                    {
                        QString articleString = textNode.text().get();
#else
            QDomDocument dd;
