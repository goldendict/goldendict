...
bool MdictParser::readHeader( QDataStream & in )
{
    ...
    quint32 checksum = 0;
    qFromLittleEndian<quint32>(file_->read( sizeof(quint32) ).data(), 1, &checksum);
...
#if QT_VERSION >= QT_VERSION_CHECK( 5, 0, 0 )
    const QRegularExpression rn( QString::fromLatin1("[\r\n]") );
#else
    const QRegExp rn( QString::fromLatin1("[\r\n]") );
#endif
#ifdef GD_PUGIXML_XSERIAL
    pugi::xml_document pdoc;
    headerText.replace(rn, QString::fromLatin1("&#10;")); // xml_document eats \r and \n
    pdoc.load_string(headerText.toUtf8().data());
    pugi::xml_node headerAttributes = pdoc.document_element();

    encoding_ = headerAttributes.attribute("Encoding").value();
    ...
    QString node_value = headerAttributes.attribute("StyleSheet").value();
    ...
    {
        //QString styleSheets = attr.value();
        QStringList lines = node_value.split( rn, QString::KeepEmptyParts );
    ...
    version_ = headerAttributes.attribute( "GeneratedByEngineVersion" ).as_double();
    if ( version_ < 2.0 )
        numberTypeSize_ = 4;
    else
        numberTypeSize_ = 8;
    ...
    // Encrypted ?
    encrypted_ = headerAttributes.attribute("Encrypted").as_int();

    // Read metadata
    rtl_ = strcmp(headerAttributes.attribute( "Left2Right" ).value(), "Yes");
    node_value = headerAttributes.attribute( "Title" ).value();
    if ( node_value.length() < 5 || node_value == "Title (No HTML code allowed)" )
    {
    ...
    {
        if ( node_value.contains( '<' ) || node_value.contains( '>' ) )
            title_ = QTextDocumentFragment::fromHtml( node_value ).toPlainText();
        else
            title_ = node_value;
    }
    node_value = headerAttributes.attribute( "Description" ).value();
    description_ = QTextDocumentFragment::fromHtml( node_value ).toPlainText();
#else
    ...
    if ( headerAttributes.contains( "StyleSheet" ) )
    {
        ...
        QStringList lines = styleSheets.split( rn, QString::KeepEmptyParts );
        for ( int i = 0; i < lines.size() - 3; i += 3 )
        {
            styleSheets_[lines[i].toInt()] = pair<QString, QString>( lines[i + 1], lines[i + 2] );
        }
