...
bool FmodexAudioPlayer::ERRCHECK(int result)
{
    if (result != FMOD_OK)
    {
        emit error( tr("FMOD error! (%1) %2").arg(result).arg(FMOD_ErrorString(FMOD_RESULT(result))) );
        return true;
    }
    return false;
}
    ...
    : fmodex(0), fmodex_dl(fmodex_dyl_name)
{
    ...
    else
    {
        ...
        result = fmodex->FMOD_System_GetVersion(fmodex->system, &version);
        if(ERRCHECK(result))
        {
            clean();
        }
        ...
        {
            result = fmodex->FMOD_System_Init(fmodex->system, 1, FMOD_INIT_NORMAL, 0);
            if(ERRCHECK(result))
                clean();
        }
    }
