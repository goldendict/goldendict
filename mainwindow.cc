...
MainWindow::MainWindow( Config::Class & cfg_ ):
    trayIcon( 0 ),
    searchPaneTitleBar(new QWidget(this)),
    groupLabel(new QLabel( searchPaneTitleBar )),
    dictsPaneTitleBar(new QWidget(this)),
    foundInDictsLabel(new QLabel(  dictsPaneTitleBar )),
    escAction( this ),
    ...
    dictionaryBar(new DictionaryBar( this, configEvents, cfg.editDictionaryCommandLine, cfg.preferences.maxDictionaryRefsInContextMenu )),
  ...
  #endif
{
    ...
    QHBoxLayout *searchPaneTitleBarLayout = new QHBoxLayout;
    ...
    QHBoxLayout *dictsPaneTitleBarLayout = new QHBoxLayout;
...
void MainWindow::editDictionaries( unsigned editDictionaryGroup )
{
    if(hotkeyWrapper)
    {
        delete hotkeyWrapper; // No hotkeys while we're editing dictionaries
        hotkeyWrapper = 0;
    }
...
void MainWindow::editPreferences()
{
    if(hotkeyWrapper)
    {
        delete hotkeyWrapper; // So we could use the keys it hooks
        hotkeyWrapper = 0;
    }
    scanPopup.reset(); // No scan popup either. No one should use dictionaries.
    closeHeadwordsDialog();
...
void MainWindow::installHotKeys()
{
    if(hotkeyWrapper)
    {
        delete hotkeyWrapper; // Remove the old one
        hotkeyWrapper = 0;
    }
         ...
         cfg.preferences.enableClipboardHotkey )
    {
        ...
        connect( hotkeyWrapper, SIGNAL( hotkeyActivated( int ) ),
...
void MainWindow::userDictNameChanged(const QString & id , const QString &uname)
{
    if(dictionaries.empty())
        return;
#ifdef GD_PUGIXML_XSERIAL
    pugi::xml_document qdd;
    pugi::xml_node root = qdd.append_child("UName");
    ...
    foreach (const sptr< Dictionary::Class > &dict, dictionaries) {
        ...
        if(name!=uname)
        {
            pugi::xml_node dn = root.append_child("map");
            dn.append_attribute("id").set_value(id.c_str());
            dn.append_attribute("name").set_value(name.c_str());
            dn.append_attribute("uname").set_value(uname.c_str());
        }
    }
    QString cfn = Config::getRenamesFileName();
    qdd.save_file(cfn.toLocal8Bit().data(), PUGIXML_TEXT(""), pugi::format_default, pugi::encoding_utf8);
#else
    QDomDocument qdd;
...
#ifdef GD_PUGIXML_XSERIAL
    pugi::xml_document qdd;
    if(!qdd.load_file(Config::getRenamesFileName().toLocal8Bit().data()))
        return;
    pugi::xml_node root = qdd.select_single_node("UName").node();
    if(root.empty())
        return;
    QMap<string, string> maps;
    pugi::xml_node::iterator i = root.begin();
    for(; i != root.end(); ++i){
        maps.insert(i->attribute("id").value(), i->attribute("uname").value());
    }
#else
    QDomDocument qdd;
